...
class CyberGuardApp {
  ...
  setupEventListeners() {
    ...
    // Health Checker Password Visibility Toggle
    const toggleCheckerBtn = document.getElementById('toggleCheckerVisibility');
    const checkerInput = document.getElementById('checkerInput');
    if (toggleCheckerBtn && checkerInput) {
      toggleCheckerBtn.addEventListener('click', () => {
        const isPassword = checkerInput.type === 'password';
        checkerInput.type = isPassword ? 'text' : 'password';
        toggleCheckerBtn.innerHTML = isPassword
